...
function socketEvents(socket, pouchCreator) {
...
function createSocketServer(server, pouchCreator, onConnect) {
  const onConnectFn = onConnect ? onConnect : function() {};
  server
...
function listen(port, options, callback) {
  ...
  if (options.attach) {
    ...
    server = io.listen(port, options.socketOptions || {}, callback);
  }
